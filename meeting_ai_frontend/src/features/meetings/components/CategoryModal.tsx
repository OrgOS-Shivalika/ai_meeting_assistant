...
export default function CategoryModal({ isOpen, onClose, category }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="bg-canvas rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative">
        <div className="px-6 pt-6 pb-4 flex items-center justify-between border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div
              className="p-2.5 rounded-xl shadow-soft flex items-center justify-center"
            ...
            <div className="grid grid-cols-4 gap-2">
              <button
                ...
                className={`px-2 py-2 rounded-lg text-xs font-semibold border transition-all ${
                  icon === "" ? "border-indigo-600 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-canvas text-slate-500 hover:border-slate-300"
                }`}
              >
                None
              </button>
              {ICON_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`px-2 py-2 rounded-lg text-xs font-semibold border transition-all ${
                    icon === opt.value ? "border-indigo-600 bg-indigo-50 text-indigo-700" : "border-slate-200 bg-canvas text-slate-600 hover:border-slate-300"
                  }`}
                >
                  {opt.label}
                </button>
            ...
            <div>
                  ...
                  editingTeamId === team.id ? (
                    ...
                    >
                      <input
                        ...
                        className="w-full px-2 py-1.5 rounded border border-slate-200 bg-canvas text-sm font-semibold focus:border-indigo-500 outline-none"
                      />
                      <textarea
                        ...
                        rows={2}
                        className="w-full px-2 py-1.5 rounded border border-slate-200 bg-canvas text-xs focus:border-indigo-500 outline-none resize-none"
                      />
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={() => setEditingTeamId(null)}
                          className="px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-slate-500 hover:text-slate-700 rounded"
                        >
                          Cancel
                        </button>
                        <button
                          ...
                          className="px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider bg-indigo-600 text-white rounded disabled:opacity-50"
                        >
                          Save
                        </button>
                      </div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="px-2 py-1 text-[10px] font-semibold uppercase tracking-wider text-indigo-600 hover:bg-indigo-50 rounded"
                        >
                          Edit
                        </button>
                        <button
              ...
              <div className="mt-3 space-y-2">
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-canvas focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none text-sm font-medium"
                />
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 rounded-lg border border-slate-200 bg-canvas focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none text-xs"
                  />
                  <button
        ...
        <div className="px-6 py-4 bg-slate-50/50 border-t border-gray-100 flex items-center justify-between gap-3">
          {isEditing ? (
            <button
              ...
              disabled={saving}
              className="px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors flex items-center gap-1.5"
            >
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onClose}
              className="px-4 py-2 border-2 border-slate-100 text-slate-600 font-semibold text-xs uppercase tracking-wider rounded-lg hover:bg-slate-50 transition-all"
            >
              Cancel
            </button>
            <button
              ...
              className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-200 text-white font-semibold text-xs uppercase tracking-wider rounded-lg shadow-soft shadow-indigo-600/20 transition-all flex items-center gap-2"
            >
