    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-600">
        ...
        <button
          ...
          className="ml-auto px-3 py-1 text-xs font-semibold bg-indigo-50 text-indigo-700 rounded hover:bg-indigo-100 disabled:opacity-50"
        >
        ...
        <ul className="space-y-2">
          {versions.map((v) => {
            ...
            return (
              <li
                key={v.id}
                className={`bg-canvas border rounded-xl p-3 flex items-center gap-3 ${
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`px-1.5 py-0.5 text-[10px] font-semibold rounded ${STATE_BADGE[v.state]}`}>
                      {v.state}
                    </span>
                    {isActive && (
                      <span className="px-1.5 py-0.5 text-[10px] font-semibold rounded bg-indigo-50 text-indigo-700 flex items-center gap-1">
                        <Check className="w-3 h-3" />
                        active
                      </span>
                    )}
                    {v.seeded_from_filesystem && (
                      <span className="px-1.5 py-0.5 text-[10px] font-semibold rounded bg-blue-50 text-blue-700">
                        seeded
                      </span>
                    )}
                    {v.eval_score !== null && (
    ...
    <div className="space-y-4 mt-4">
      ...
      {sections.length > 0 && (
        <div className="space-y-3">
          <h4 className="text-xs font-semibold text-slate-600">Modular prompt sections</h4>
          {sections.map((key) => {
            ...
            return (
              <div key={key} className="bg-slate-50 border border-slate-200 rounded-lg overflow-hidden">
                <div className="px-3 py-2 text-xs font-semibold text-slate-700 bg-slate-100">
                  {key}
                </div>
      ...
      {retrievalKeys.length > 0 && (
        <div>
          <h4 className="text-xs font-semibold text-slate-600 mb-2">Retrieval config</h4>
      ...
      {modelKeys.length > 0 && (
        <div>
          <h4 className="text-xs font-semibold text-slate-600 mb-2">Model config</h4>
          <ul className="text-xs font-mono space-y-1">
            {modelKeys.map((k) => {
      ...
      {!noToolChanges && (
        <div>
          <h4 className="text-xs font-semibold text-slate-600 mb-2">Tool permissions</h4>
