"""Database logic for the graph read API (Phase 3D).

Extracted from ``app/api/graph_router.py`` so the router stays a thin
transport layer. Functions take the SQLAlchemy ``Session`` plus the current
user and raise ``HTTPException`` for tenant-isolation / not-found failures —
...
- ``list_entities`` and ``get_entity`` bump ``last_accessed_at`` and
  ``access_count`` on the entities they return.
- ``get_meeting_graph`` does NOT bump — it's a debug/admin view and we
