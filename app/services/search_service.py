"""Database logic for the Phase 2D / 4E search API.

Extracted from ``app/api/search_router.py`` so the router stays a thin
transport layer. Functions take the SQLAlchemy ``Session`` plus the current
user and raise ``HTTPException`` for scope / ownership failures — mirroring
the convention in ``category_service`` and keeping behaviour identical to the
previous in-router helpers.

Phase 4E: the search is polymorphic over `meeting_chunks` and
`document_chunks`. Both tables carry a 1536-d `embedding` column with the
same HNSW + cosine_ops index, so we issue two parallel pgvector queries
(each top-K under the HNSW path) and merge by distance in Python. This
avoids fighting SQLAlchemy UNION semantics with mismatched column types,
while still letting each table's HNSW index do the heavy lifting. The
`sources` body param can narrow the union to one side.
"""
...
def validate_scope(db: Session, user, scope: str, scope_id: int | None) -> None:
    if scope == "category":
        cat = db.execute(
...
# ---------------------------------------------------------------------------
# Search orchestration — query both sources, merge, bump access counts,
# and log 6B events.
# ---------------------------------------------------------------------------
...
def search_chunks(
    ...
    if payload.sources in ("all", "meetings"):
        meeting_hits, _ = _search_meeting_chunks(db, user, payload, query_vec)
    if payload.sources in ("all", "documents"):
        document_hits, _ = _search_document_chunks(db, user, payload, query_vec)
...
# ---------------------------------------------------------------------------
# Inspection endpoints — full chunk listings for a meeting / document.
# ---------------------------------------------------------------------------
...
def get_document_chunks(
    db: Session, user, kind: str, document_id: UUID,
