...
def fetch_agent_traces(agent_slug: str, limit: int = 50) -> dict:
    ...
    Shape:
        {
            "enabled": bool,
            "host":    "https://cloud.langfuse.com" | None,
            "traces":  [ { id, timestamp, name, session_id, latency,
                           total_cost, input_tokens, output_tokens }, ... ],
            "error":   str | None,
        }

    Failures are swallowed and surfaced via `error` so the frontend can
    render a graceful empty state instead of a 500.
    """
    if not _LANGFUSE_ENABLED:
        return {"enabled": False, "host": None, "traces": [], "error": None}
    try:
        host = settings.LANGFUSE_HOST or "https://cloud.langfuse.com"
        result = _LANGFUSE_CLIENT.fetch_traces(tags=[agent_slug], limit=limit)
        traces = []
        for t in result.data:
            # SDK field names differ slightly between versions — read defensively.
            latency = getattr(t, "latency", None)
            total_cost = getattr(t, "total_cost", None)
            if total_cost is None:
                total_cost = getattr(t, "totalCost", None)
            traces.append({
                "id": t.id,
